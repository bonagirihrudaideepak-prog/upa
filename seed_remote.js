...
function login() {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      hostname:'upanishad-store.onrender.com',
...
async function seedRemote() {
  console.log('Logging into live Render server...');
  const token = await login();
  console.log('Token received. Populating 100 products...');

  const items = [];

  iphoneModels.forEach((m, i) => items.push({
    name: m,
    price: 49999 + (20 - i) * 3500,
    category: 'iPhone',
    sku: `IPH-10${i + 1}`,
    description: `Original ${m} with official store takeaway warranty and genuine packaging.`,
    stock: 12,
    is_featured: i < 8,
    is_new_arrival: i < 8,
    is_offer: i % 2 === 0,
  ...
  samsungModels.forEach((m, i) => items.push({
    name: m,
    price: 24999 + (20 - i) * 3000,
    category: 'Samsung',
    sku: `SAM-10${i + 1}`,
    description: `Official ${m} with official Samsung warranty & instant takeaway.`,
    stock: 15,
    is_featured: i < 8,
    is_new_arrival: i < 8,
    is_offer: i % 3 === 0,
  ...
  accessoriesList.forEach((m, i) => items.push({
    name: m,
    price: 399 + i * 80,
    category: 'Accessories',
    sku: `ACC-10${i + 1}`,
    description: `Premium grade ${m} with high durability and precise cutouts.`,
    stock: 50,
    is_featured: i < 8,
    is_new_arrival: i < 8,
    is_offer: i % 2 === 1,
    likes_count: 80 + i * 15,
    image_url: 'https://images.unsplash.com/photo-1603313011101-320f26a4f6f6?w=600&auto=format&fit=crop'
  }));

  gadgetsList.forEach((m, i) => items.push({
    name: m,
    price: 699 + i * 250,
    category: 'Gadgets',
    sku: `GAD-10${i + 1}`,
    description: `Next-gen ${m} featuring fast charge and intelligent battery management.`,
    stock: 30,
    is_featured: i < 8,
    is_new_arrival: i < 8,
    is_offer: i % 3 === 1,
  ...
  othersList.forEach((m, i) => items.push({
    name: m,
    price: 22999 + i * 3500,
    category: 'Others',
    sku: `OTH-10${i + 1}`,
    description: `Flagship ${m} with complete accessories package & store warranty.`,
    stock: 10,
    is_featured: i < 8,
    is_new_arrival: i < 8,
    is_offer: i % 2 === 0,
  ...
  for (const item of items) {
    const ok = await createProduct(token, item);
    if (ok) successCount++;
  }
  console.log(`Live Render Seeding Complete: ${successCount}/${items.length} products created!`);
}
